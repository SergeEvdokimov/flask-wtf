...
def show_cabin(sex, age):
    if sex == 'male':
        ...
        color = '#FF0000'
    if age < 21:
        pic = url_for('static', filename='img/kid.png')
    else:
        pic = url_for('static', filename='img/adult.png')
    url_style = url_for('static', filename='css/table.css')
    return render_template('7.html', url_style=url_style, color=color, pic=pic)
